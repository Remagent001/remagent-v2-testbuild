    ...
    recipients: {
      signers: [{
        ...
        tabs: {
          signHereTabs: [{ anchorString: "/sn1/", anchorUnits: "pixels", tabLabel: "signature" }],
          textTabs: [
            { anchorString: "/name/", anchorUnits: "pixels", anchorMatchWholeWord: "true", tabLabel: "name", value: signerName, locked: "false", width: 250, height: 15 },
            { anchorString: "/addr/", anchorUnits: "pixels", anchorMatchWholeWord: "true", tabLabel: "address", value: address || "", locked: "false", width: 250, height: 15 },
          ],
          dateSignedTabs: [{ anchorString: "/date/", anchorUnits: "pixels", tabLabel: "dateSigned" }],
        },
      }],
...
export async function getSignedDocument(envelopeId) {
  const token = await getAccessToken();
  const baseUrl = process.env.DOCUSIGN_BASE_URL || "https://demo.docusign.net/restapi";
  const accountId = process.env.DOCUSIGN_ACCOUNT_ID;
